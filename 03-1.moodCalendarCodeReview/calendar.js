...
function makeDays(month) {
  ...
  for (var i = 1; i <= maxDate + emptySpace; i++) {
    var day = i - emptySpace;

    const month_days = document.getElementById(`${month}month_days`);

    var circle;
    if (i <= emptySpace) circle = `<div class="days"><span class="circle empty"></span></div>`;
    else circle = `<div class="days"><span class="circle" id="${month}-${day}">${day}</span></div>`;
